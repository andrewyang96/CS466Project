...
def mutate_sequence(seq): #more deletions (most of the time)
    #print(seq)
    length = len(seq)
    #print("original_length is " + str(length))
    num_mutations = int(round(length/2))
    #print(num_mutations)
    for i in range(num_mutations):
        #print("length is " + str(len(modified_seq)))
        mutationIdx = random.randint(0, (len(seq) - 1))
        if random.uniform(0,1) <= 0.05: # random probability of mutation
            while True:
...
def mutate_sequence2(seq):
    ...
    for i in range(num_mutations):
        #print("length is " + str(len(modified_seq)))
        mutationIdx = random.randint(0, (len(seq) - 1))
        if random.uniform(0,1) <= 0.50: # random probability of mutation
            while True:
                random_int = random.randint(0,3)
                if(seq[mutationIdx] != nucleotides[random_int]): #makes sure nucleotide mutates to a different one
                    #print("mutate: " + str(mutationIdx))
...
if __name__ == '__main__':
    ...
    with open(fasta_file, "rU") as handle:
        for record in SeqIO.parse(handle, "fasta"):
            ...
            S3 = mutate_sequence2(list(''.join(record.seq)))
